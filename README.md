...
- `DataStore.syncReconcile()` is the **only** correct way to push local changes to the sheet. It reads the sheet back and does a 3-way merge per cycle (keyed by `id`, falling back to `startDate` for pre-id rows) against `periodTracker_lastSyncedRows` — a snapshot of what the app itself last wrote. Whichever side (cloud or local) actually changed since that baseline wins, which is what lets a manual edit directly in the sheet (including shrinking `periodDayCount`, adding a row for a missed period, or deleting a bad row) stick instead of being reverted on the next sync. Only *then* does it write the merged result back.
...
8. **Manual edits made directly in the Google Sheet were always reverted on the next app open** — `syncReconcile()` used to keep whichever side had the larger `periodDayCount`, a rule that could only add data, never accept a correction. This blocked the sheet from ever being usable to fix historical mistakes (missed logs, wrong period lengths) older than the in-app editor's 90-day window. Fixed by replacing the "larger wins" rule with a 3-way merge against `periodTracker_lastSyncedRows` (a snapshot of what the app itself last wrote) — see the "Sync model" section above.
