...
- `DataStore.syncReconcile()` is the **only** correct way to push local changes to the sheet. It reads the sheet back, merges with local as a union keyed by `startDate` (keeping the larger `periodDayCount` on conflict — a rule that can only add data, never remove it), and only then writes the merged result back.
